...
### Stacked PR Workflow
1. **Start a new feature**: `gt create -m "feat: base implementation"`
2. **Add incremental changes**: Make changes, then `gt create -m "feat: add tests"` for each logical chunk
3. **Submit for review**: `gt ss` pushes all branches and creates/updates PRs
4. **After review feedback**: Make changes, `gt modify`, then `gt ss` to update
...
### PR Descriptions
Graphite is configured to use commit messages as PR bodies. When creating commits, include the PR description as the commit body using a second `-m` flag:

```bash
gt create -m "feat: add new feature" -m "## Summary
